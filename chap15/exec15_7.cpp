...
class Disc_quote: public Quote {
    public:
        Disc_quote() = default;
        Disc_quote(const string &book, double p, size_t qty, double disc): Quote(book, p), disc_qty(qty), discount(disc) { }
        double all_price(size_t cnt) const override
        {
            if (cnt <= disc_qty)
            {
            ...
            {
                size_t more = cnt - disc_qty;

                return disc_qty * (1 - discount) * price + more * price;
            }
        }
...
main(int argc, char *argv[])
{
    ...
    Disc_quote dq("isbn003", 1, 100, 0.1);
    print_all(cout, dq, 101);
    return 0;
}
